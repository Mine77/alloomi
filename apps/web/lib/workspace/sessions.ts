...
export function listSessionFiles(
  ...
  // Sort: directories first, files last, sorted by name
 ...
 */
export async function getAllWorkspaceFilesRecursive(): Promise<
  WorkspaceFileItem[]
> {
  const taskIds = listAllSessions();
  ...
  for (const taskId of taskIds) {
    ...
    await traverseDir(sessionDir, "", taskId);
  }
  ...
  async function traverseDir(
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          await traverseDir(fullPath, fileRelativePath, taskId);
        }
      }
    } catch (err) {
