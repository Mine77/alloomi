...
import { getActiveListenerClientBySession } from "@/lib/telegram/user-listener";
import {
  delay,
  isEmptyMessage,
  timeBeforeHours,
...
import type { ContactMeta, TelegramContactMeta } from "@/lib/types/contacts";
...
export class TelegramAdapter extends MessagePlatformAdapter {
  ...
  constructor(opts?: {
    ...
    // Prefer to reuse connected client from User Listener, avoid creating multiple MTProto connections for same session
    ...
    const sharedClient =
      !opts?.botToken && sessionKey
        ? getActiveListenerClientBySession(sessionKey)
        : undefined;
  ...
  private async extractAttachmentsFromMessage(
    ...
    const pushAttachment = async (
      ...
      if (!ingestResult.success) {
        console.warn(
          `[Bot ${this.botId}] [telegram] Skipped attachment due to ${ingestResult.reason} mimeType=${mimeType} fileName=${fileName}`,
        );
        return;
      }
