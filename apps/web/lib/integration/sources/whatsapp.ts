...
import { timeBeforeHours } from "@/lib/utils";
import { ingestAttachmentForUser } from "@/lib/integration/utils/attachments";
...
type LoginDeferred = {
  ...
  // Stored separately so we can call reject even after firstQrResolved
...
export class WhatsAppAdapter extends MessagePlatformAdapter {
  ...
  private authState: WhatsAppBaileysAuthState;
  ...
  constructor(opts?: {
    ...
    this.authState = new WhatsAppBaileysAuthState(this.sessionId);
    ...
    } = require("@/lib/whatsapp/client-registry");
    let existingSock = whatsappClientRegistry.get(this.botId);
    ...
    if (!existingSock && this.botId) {
      ...
      if (existingAdapter?.sock) {
        existingSock = existingAdapter.sock;
        if (DEBUG)
          console.log(
  ...
  setRegisterSocketAs(key: string): void {
    ...
    if (sock) {
      ...
      whatsappClientRegistry.register(key, sock);
      if (DEBUG)
        console.log(
  ...
  private async createSocket(): Promise<WASocket> {
    ...
    const auth = await this.authState.ensureAuthState();
    if (DEBUG)
      console.log(
    ...
    if (this.botId) {
      ...
      whatsappClientRegistry.register(this.botId, sock);
    }
    ...
    const connHandler = (update: Partial<ConnectionState>) => {
      ...
      if (connection === "close") {
        ...
        if (this.botId) {
          ...
          whatsappClientRegistry.unregister(this.botId);
        }
      }
    };
    ...
    const credsHandler = () => {
      if (this.isAuthenticated) {
        void this.authState
          ...
          .catch((e) =>
            DEBUG
              ? console.error("[whatsapp] saveCreds failed:", e)
              : undefined,
          );
      }
    };
  ...
  private async ensureReady(): Promise<void> {
    ...
    if (!this.initializationPromise) {
      this.initializationPromise = (async () => {
        ...
        const exists = await this.authState.exists();
        if (DEBUG)
          console.log(
        ...
        if (this.botId && whatsappClientRegistry.has(this.botId)) {
          const existing = whatsappClientRegistry.get(this.botId);
          if (existing) {
            console.log(
  ...
  async getChatsByHours(hours = 1): Promise<ExtractedMessageInfo[]> {
    return this.getChatsByTime(timeBeforeHours(hours));
  }

  async getChatsByDays(days = 1): Promise<ExtractedMessageInfo[]> {
    return this.getChatsByTime(timeBeforeHours(days * 24));
  }
  ...
  async sessionExists(): Promise<boolean> {
    return await this.authState.exists();
  }
  ...
  async ingestMessageAttachments(msg: WAMessage): Promise<Attachment[]> {
    if (!this.ownerUserId || !this.ownerUserType) return [];
