...
import { CLAUDE_METADATA, defineAgentPlugin } from "@alloomi/agent/plugin";
import type { AgentPlugin } from "@alloomi/agent/plugin";
import type {
  AgentConfig,
  AgentMessage,
  AgentOptions,
...
function spawnClaudeCodeProcess(options: {
  ...
  if (resolvedCwd) {
    ...
    if (!isAbsolute) {
      ...
      console.log("[Claude] Converted relative cwd to absolute:", resolvedCwd);
    }
  }
...
async function ensureClaudeCode(): Promise<string | undefined> {
  console.log("[Claude] ensureClaudeCode() called");
...
1. **chatInsight** - Use for queries about:
   - "今天的事情有哪些" / "What are my tasks today"
   - "我的待办" / "My todos" / "My tasks"
   - "重要事项" / "Important items" / "Priorities"
   - "聊天记录" / "Chat history" / "Conversations"
   - "项目进展" / "Project updates"

2. **queryContacts** - Use for queries about:
   - "我的联系人" / "My contacts" / "Who is X"
...
3. **queryIntegrations** - Use for queries about:
   - "我的账号" / "My accounts" / "What platforms are connected"
...
4. **searchKnowledgeBase** - Use for queries about:
   - "我的文档" / "My documents" / "Knowledge base"
...
6. **createInsight** - Use when user says:
   - "创建一个提醒" / "Create a reminder"
   - "记录这个" / "Remember this"
   - "我想追踪这个" / "I want to track this"
...
7. **createScheduledJob** - Use when user asks for RECURRING or SCHEDULED tasks:
   - "每几个小时帮我做一下..." / "Do this every few hours..."
   - "每天早上提醒我..." / "Remind me every morning..."
   - "每周执行这个任务" / "Run this task every week..."
   - "每小时检查一次" / "Check every hour..."
   - "定时发送通知" / "Send notification periodically..."
   - "每小时、每天、每周" / "Every hour, daily, weekly..."
   - "每隔X时间" / "Every X time..."
   ...
   **CRITICAL: The 'description' field MUST preserve the user's exact original request:**
   ...
   - Keep the recipient identity (user says '我', use '我' NOT '用户')
...
8. **modifyInsight** - Use when:
   ...
   - User says "更新" / "update" or "加一条更新" / "add an update" referring to an existing tracking
...
9. **listScheduledJobs** - Use when:
   - User asks "列出我的定时任务" / "list my scheduled jobs"
...
10. **updateScheduledJob** - Use when:
    ...
    - User says "更新定时器" / "update timer" or "修改定时任务" / "modify scheduled job"
...
11. **sendReply** - Use when user says:
   ...
   **IMPORTANT WORKFLOW for sending messages/replies:**
   ...
   Examples:
   - User: "回复张三，告诉他..." → Query "张三" from contacts → Check platforms → Send reply
   - User: "给李四发个消息" → Query "李四" from contacts → Check platforms → Send message
   - User: "告诉team..." → Query team contacts → Check integrations → Send message
...
export class ClaudeAgent extends BaseAgent {
  ...
  private buildEnvConfig(): Record<string, string> {
    ...
    // Priority: environment variable ANTHROPIC_BASE_URL takes precedence over config.baseUrl
    const envBaseUrl = env.ANTHROPIC_BASE_URL;
    if (envBaseUrl) {
      // Environment variable ANTHROPIC_BASE_URL is configured, preserve it
      if (this.config.apiKey) {
        env.ANTHROPIC_AUTH_TOKEN = this.config.apiKey;
      }
      // Don't override ANTHROPIC_BASE_URL - keep the environment variable value
    } else if (this.config.apiKey) {
      // No environment variable, use modelConfig
      env.ANTHROPIC_AUTH_TOKEN = this.config.apiKey;
      ...
      if (this.config.baseUrl) {
        ...
        // Neither config nor env var, use default Anthropic API
...
${formattedMessages}${truncationNotice}\n\n---\n## Current Request\n`;
  ...
  async *run(
    ...
    try {
      const { syncSkillsToClaude } = require("@/lib/skills/loader");
    ...
    if (hasPDFs) {
      ...
      pdfs.forEach((pdf, i) => {
        const sizeMB = (pdf.data.length * 0.75) / (1024 * 1024); // Approximate base64 to bytes
        console.log(
          `[Claude ${session.id}] PDF ${i}: mimeType=${pdf.mimeType}, pageCount=${pdf.pageCount || "unknown"}, estimatedSize=${sizeMB.toFixed(2)}MB`,
        );
      });
    ...
    try {
      ...
      if (process.platform === "win32") {
        try {
          const { clearSkillsFromClaude } = require("@/lib/skills/loader");
...
`;
    ...
    const queryOptions = {
      ...
      // with a 5-minute TTL, saving 60-90% of input tokens for repeated turns (#1496)
      systemPrompt: PLANNING_INSTRUCTION(),
    } as Options;
  ...
  async *execute(options: ExecuteOptions): AsyncGenerator<AgentMessage> {
    ...
    const executionPrompt = `${formatPlanForExecution(plan, sessionCwd, sandboxOpts, userAiSoulPrompt, userLanguage)}\n\nOriginal request: ${options.originalPrompt}`;
    logger.info(
  ...
  private sanitizeText(text: string): string {
    ...
    const hasTimeout = /timeout|TIMEDOUT|ETIMEDOUT|超时/i.test(sanitized);
