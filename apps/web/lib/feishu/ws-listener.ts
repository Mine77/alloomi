...
type FeishuCredentials = { appId?: string; appSecret?: string };
...
/** Deduplicate processed messages. deduplicate before dispatch to avoid duplicate replies from redelivery) key = `${accountId}:${messageId}`, value = processing timestamp */
...
export async function startFeishuConnection(
  ...
  const eventDispatcher = new Lark.EventDispatcher({}).register({
    "im.message.receive_v1": async (data: {
      ...
      // Deduplicate before dispatch: mark as processed only here to avoid duplicate replies from Feishu redelivery or dual events
      ...
      const auth = { authToken: conn.authToken };
