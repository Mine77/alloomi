 ...
 */
async function removeFile(fsPath: string, isDirectory: boolean): Promise<void> {
  if (!isTauriMode()) return;

  try {
    await fs.promises.rm(fsPath, { recursive: true, force: true });
  } catch (error) {
    console.error("[FS-Sync] Failed to remove:", fsPath, error);
  }
}
 ...
 */
async function moveFile(oldFsPath: string, newFsPath: string): Promise<void> {
  if (!isTauriMode()) return;
  // Ensure target directory exists
  const dirPath = path.dirname(newFsPath);
  await createDirectory(dirPath);
